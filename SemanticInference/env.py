...
class Env(object):
	...
	def __init__(self, dataPath, task=None):
		...
		self.entity2vec = np.loadtxt(dataPath + 'entity2vec.bern')		# 75492*100
		self.relation2vec = np.loadtxt(dataPath + 'relation2vec.bern')
		# print("shape: ", self.entity2vec.shape)
		...
		f = open(dataPath + 'kb_env_rl.txt')		# 30w 条三元组
		kb_all = f.readlines()
		f.close()
		...
		if task != None:
			...
			for line in kb_all:
				...
				if rel != relation and rel != relation + '_inv':
					self.kb.append(line)		# kb_all中和task关系不同的所有关系对应的三元组
